...
# show_lidar(load_lidar_data("
# c=load_joint_data("E:\\ESE 650\HW3\\ese650_hw3_p2\\p2\\data\\train\\train_joint0.mat")
# a=np.zeros((len(c),3))
...
#         #N Random numbers
#         u_k = (np.arange(0,p.shape[0]) + np.random.uniform(size=p.shape[0]))/ p.shape[0]
...
#         #Resampling 
#         while(i<len(p.shape[0])):
#             while(w[j]<u_k[i]):
#                 j+=1
#             ns[:,i]=u_k[:,j]
#             i+=1
        
#         #Assigning the new samples to the original variable
#         p=ns
#         w=np.ones(p.shape[0])/[p.shape[0]]
        # return p,w

# stratified_resampling(a, w)
# print(len(c["head_angles"][0]))